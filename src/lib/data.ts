...
export type Artwork = {
  ...
  /** Optional curated NSFW flag. When absent, the listing projection falls
   *  back to the title heuristic in `src/lib/nsfw.ts`. Stored on the full
   *  record so future hand-review can override the heuristic (true to
   *  flag, false to whitelist a false positive). */
  nsfw?: boolean | null;
};
...
export type ArtworkListing = Pick<
  ...
  | "realDimensions"
> & {
  /** Computed at module load from the title heuristic (or the curated
   *  `Artwork.nsfw` override when present). Always present on the listing
   *  so client components don't need to import the heuristic. */
  nsfw: boolean;
};
...
const _artworkListings: ArtworkListing[] = (artworksJson as Artwork[]).map((a) => ({
  ...
  nsfw: a.nsfw === true || (a.nsfw == null && isNsfwTitle(a.title)),
}));
...
export function getArtworkListingsByArtist(slug: string): ArtworkListing[] {
  return artworkListings.filter((a) => a.artistSlug === slug);
}
