...
export type Artwork = {
  ...
  /** True when the artwork was flagged by `scripts/tag-nsfw.mjs` —
   *  either via Wikidata `depicts` (P180) pointing at a nudity-related
   *  QID, or a keyword match on filename/title/description. The UI uses
   *  this to apply the user's NSFW preference (hide / blur / show);
   *  field is omitted (undefined) for everything else. */
  nsfw?: boolean;
};
