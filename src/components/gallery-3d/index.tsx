...
export function Gallery3D({ artworks }: Props) {
  ...
  // Track the exact owner for the HUD / minimap.
  const handleRoomChange = useCallback((owner: number) => {
    ...
    <div ref={galleryHostRef} className="relative w-full h-screen bg-black">
      ...
      >
        ...
        {/* Global fill for non-active rooms. Tuning is a balance between
            ...
            world-up regardless. */}
        <ambientLight intensity={0.202} />
        <hemisphereLight args={["#fff3d0", "#2a1f15", 0.154]} position={[0, 20, 0]} />
...
function FloorScene({
  ...
  // Every room's shell (walls / floor / ceiling) renders as soon as the
  // floor mounts — shells are cheap, and rendering them all keeps the
  // building solid (no voids through the enfilade doorways). The
