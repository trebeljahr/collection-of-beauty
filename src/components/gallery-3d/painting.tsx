...
//
// The top two tiers share an upgrade radius (sq(1.05) — slight margin
// around 1 m so a player standing exactly on the 1 m line still commits
// to the upgrade). They are NOT nested; the tier-pick order in the
// picker decides which one displays.
//
...
type LodTier = {
  ...
  downgradeSq: number;
  releaseSq: number;
  /** Pre-built variant width this tier loads. */
  width: number;
};
...
// The in-scene ladder stops at 4096 px. There used to be an "original"
// tier above it that decoded the per-source full-size AVIF (capped at
// 8192 px) so the walkable scene reached true source detail at 1 m. It
...
//
// Pixel-peeping still works: the zoom modal loads the full-size variant
// itself as a plain <img> (see zoom-modal.tsx `highSrc`) and only uses
// the cached scene texture as an instant placeholder under the cross-
// fade. All 967 big-source works have a 4096 px variant, so the top of
// the scene ladder is populated for every one of them.
const LOD_TIERS: LodTier[] = [
  { width: 4096, ...TOP_TIER_BANDS },
  { width: 2560, ...TOP_TIER_BANDS },
  {
...
function PaintingPlane({
  ...
  useEffect(() => {
    ...
    const tiers = LOD_TIERS.filter((t) => available.has(t.width) && t.width > baseWidth);
