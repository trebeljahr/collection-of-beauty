...
// lower tier's. The "original" tier has the tightest window — only
// used when the player is right up against the canvas — because the
// original can be a multi-MB JPEG, much heavier than the AVIFs.
// A player walking a hallway at ~3 m/s crosses each band in
...
const LOD_TIERS: LodTier[] = [
  ...
  // "press E" loads in the zoom modal for these paintings, so getting
  // up close in the 3D gallery now reaches the same source detail.
  // Distance band sits inside the 4096 band so the cheap AVIF carries
  // the player most of the way in; the original only commits to its
  // download once the player has clearly stopped to study the work.
  {
    kind: "original",
    prefetchSq: sq(0.9),
    upgradeSq: sq(0.4),
