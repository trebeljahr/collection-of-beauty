...
// Internal verification panel for the duplicate-image sweep.
// Dev-only: 404s outside `next dev`, blocked in robots.ts, not in sitemap,
// not linked from the site nav. Reads metadata/duplicate-images.json
// (produced by scripts/find-duplicate-images.mjs) and renders every
// cluster as a side-by-side image grid so the curator can eyeball which
// "same artist + same title" pairs are genuine duplicates vs distinct works.
...
export default async function DedupReviewPage() {
  ...
  // this short-circuits to 404 before reading any data — the metadata
    ...
    <main className="mx-auto max-w-7xl space-y-10 p-6">
      <header className="space-y-2">
        ...
        <p className="text-xs text-zinc-500">
          Dev-only. Red = min Δ ≤ 5 (likely duplicate scan). Orange = 6–10. Amber = 11–15. White =
          high Δ (probably a series sharing a title). Click an image to open the artwork page in a
          new tab. Tell Claude which IDs to drop or which titles to override.
        </p>
      </header>
