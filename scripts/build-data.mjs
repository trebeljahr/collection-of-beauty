...
// NSFW tags generated by `node scripts/tag-nsfw.mjs`. Optional —
// if the file isn't present, no artworks get flagged and the UI
// renders everything by default. Keyed by the same filename used
// in the per-folder metadata files.
async function loadNsfwTags() {
  const p = path.join(META, "nsfw-tags.json");
  if (!existsSync(p)) {
    console.log(`[build-data] nsfw tags: skipped (no metadata/nsfw-tags.json)`);
    return new Set();
  }
  const raw = JSON.parse(await readFile(p, "utf8"));
  const s = new Set(Object.keys(raw));
  console.log(`[build-data] nsfw tags: ${s.size} flagged`);
  return s;
}
...
async function main() {
  ...
  const nsfwSet = await loadNsfwTags();
  ...
  async function pushFromFolder(folderKey, data) {
    for (const [fname, entry] of Object.entries(data.entries)) {
      ...
      const isNsfw = nsfwSet.has(fname);
      artworks.push({
        id,
        title,
        artist: artistName,
