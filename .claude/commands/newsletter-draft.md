...
- **`excerpt`**: one or two sentences, 95–155 chars. Used for OG tags, RSS and the archive index. Name specific things in specific pictures, not why the issue is beautiful. Three constraints, all of which the first two dozen issues violated and had to be rewritten: (1) **never open with "Five …"** or enumerate all five works — every excerpt read `Five X: a, b, c, d and e`, which as a set is unmistakably generated; (2) **do not reuse the body's opening sentence**, since the body sits directly under it on the edition page; (3) **vary the shape against the previous few issues** — some a single claim, some two paired observations, some a run of four. Read the last five excerpts together before writing a new one; a repeated skeleton is the tell, not any one line.
- **`tags`**: 2–4 lowercase kebab-case tags. E.g. `ukiyo-e`, `weather`, `edo-period`.
- **`cover`**: leave it pointing at the lead artwork (the first id), or swap to the work that has the strongest single image at small sizes. The `/drops` card keeps the cover's own shape between 4:5 and 3:2 (square to 16:9 on full-width phone cards) and crops anything taller or wider; for those covers, add `focus: { x, y }` (percent of width and height) on the point that must stay in frame, e.g. a face near the top of a hanging scroll.
...
- the three CLI commands the user runs next:
  ...
  - `pnpm newsletter:send <fileSlug> --confirm` — sends to `LISTMONK_TEST_LIST_ID` (any non-production NODE_ENV picks the test list)
  - `NODE_ENV=production pnpm newsletter:send <fileSlug> --confirm` — real send to `LISTMONK_LIST_ID`
