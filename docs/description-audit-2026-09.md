...
> **Span-edit pass, 2026-09-13.** The 208 single-reviewer `minor` proposals were **not** applied as rewrites. Each finding was turned into minimal `delete` / `replace` edits on the exact flagged words, drafted by Sonnet. An independent Opus reviewer then checked every edit against the image: 196 accepted, 88 amended, 10 rejected, 10 added. A deterministic script applied the surviving edits, so no text outside a verified edit changed. 202 descriptions changed; 6 had no edit survive. Results keep a median 89% of the original words (the rewrites kept 75%) and add a median of 2 new words. 7 add a date, museum or size, each a sourced correction; the rewrites would have added unchecked ones in 52. Per-edit decisions and reasons are in each entry's `edits[]`; the rewrite in `proposedDescription` is kept for reference but was not used. 48 issues stay open as `no-false-claim` (the text never says what is shown).
