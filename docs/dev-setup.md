# Dev URL setup (`https://collection-of-beauty.local.ricoslabs.com/`)
...
```
https://collection-of-beauty.local.ricoslabs.com/
```
...
This project's slug is **`collection-of-beauty`**, recorded in
`.hatchkit.json` under `localDev.slug`. When `pnpm dev` starts, the
hatchkit dev plugin:

1. Reads the slug + the live dev port from the running server.
2. Writes/updates `~/.config/dev/projects/collection-of-beauty.caddy` pointing at
...
➜  Tailscale: https://collection-of-beauty.local.ricoslabs.com/
```
...
…also removes `~/.config/dev/projects/collection-of-beauty.caddy`. Other projects'
fragments stay put.
